...
## Best fit for Gemini
Gemini's large context window is well-suited for:
- Whole-codebase reviews (act as `reviewer`)
- Reading long specifications and synthesizing across them (act as `architect`)
- Cross-document discovery analysis (act as `product`)
